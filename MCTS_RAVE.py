...
class Node:
    __slots__ = ("game", "parent", "move", "children", "visits", "visits_rave",
                 "wins", "wins_rave", "untried_moves", "sim_moves")
    ...
    # compute the RAVE value for this node, which combines the standard win rate with the RAVE win rate for the move, weighted by the number of visits
    def rave(self) -> float:
        if self.visits == 0:
            return float("inf")
    ...
    def best_child(self) -> "Node":
        return max(self.children, key=lambda n: n.rave())
        ...
        while not sim.is_terminal() and depth < MAX_DEPTH:
            ...
            for s in sim.alive_snakes():
                ...
                if s.id == sim.my_id:
                    ...
                    self.sim_moves.append((move, s.id))
                else:
    ...
    # and also update RAVE stats for all moves that were used in the rollout
...
def choose_mcts_rave_move(game_state: typing.Dict) -> str:
